...
def getAnnoLut(iNansat,iPol,iProd):
    ...
    for iVector in vectorList.getElementsByTagName(iProd+'Vector'):
        oLUT['pixel'].append(\
            map(int,getValue(getElem(iVector,['pixel'])).split()))
        oLUT['line'].append(int(getValue(getElem(iVector,['line']))))
        oLUT['value'].append(\
            map(float,getValue(getElem(iVector,[productDict[iProd]])).split()))
    return oLUT
...
class Sentinel1Image(Nansat):
    ...
    def __getitem__(self, bandID):
        ...
        startTime = convertTime2Sec(getValue(getElem(xmldocElem,['startTime'])))
        azimuthTimeInterval = \
            float(getValue(getElem(xmldocElem,['azimuthTimeInterval'])))
        numberOfSamples = int(getValue(getElem(xmldocElem,['numberOfSamples'])))
        numberOfLines = int(getValue(getElem(xmldocElem,['numberOfLines'])))
        ...
        for iOrbit in orbitList.getElementsByTagName('orbit'):
            orbit['time'].append(\
                convertTime2Sec(getValue(getElem(iOrbit,['time']))))
            orbit['px'].append(float(getValue(getElem(iOrbit,['position','x']))))
            orbit['py'].append(float(getValue(getElem(iOrbit,['position','y']))))
            orbit['pz'].append(float(getValue(getElem(iOrbit,['position','z']))))
            orbit['vx'].append(float(getValue(getElem(iOrbit,['velocity','x']))))
            orbit['vy'].append(float(getValue(getElem(iOrbit,['velocity','y']))))
            orbit['vz'].append(float(getValue(getElem(iOrbit,['velocity','z']))))

        azimuthFmRate = { 'azimuthTime':[], 't0':[], 'c0':[], 'c1':[], 'c2':[] }
        azimuthFmRateList = getElem(xmldocElem,['azimuthFmRateList'])
        for iAzimuthFmRate in \
            azimuthFmRateList.getElementsByTagName('azimuthFmRate'):
            azimuthFmRate['azimuthTime'].append( \
                convertTime2Sec(getValue(getElem(iAzimuthFmRate,['azimuthTime']))))
            azimuthFmRate['t0'].append( \
                float(getValue(getElem(iAzimuthFmRate,['t0']))))
            tmpValues = getValue(getElem( \
                            iAzimuthFmRate,['azimuthFmRatePolynomial'])).split(' ')
        ...
        for iAntPat in antPatList.getElementsByTagName('antennaPattern'):
            subswathID = getValue(getElem(iAntPat, ['swath']))
            antennaPatternTime[subswathID].append( \
                convertTime2Sec(getValue(getElem(iAntPat,['azimuthTime']))))
        ...
        for iGeoGridPt in \
            ...
            geolocationGridPoint['azimuthTime'].append( \
                convertTime2Sec(getValue(getElem(iGeoGridPt,['azimuthTime']))))
            geolocationGridPoint['slantRangeTime'].append( \
                float(getValue(getElem(iGeoGridPt,['slantRangeTime']))))
            geolocationGridPoint['line'].append( \
                float(getValue(getElem(iGeoGridPt,['line']))))
            geolocationGridPoint['pixel'].append( \
                float(getValue(getElem(iGeoGridPt,['pixel']))))
            geolocationGridPoint['elevationAngle'].append( \
                float(getValue(getElem(iGeoGridPt,['elevationAngle']))))
        ...
        replicaTime = convertTime2Sec(getValue(getElem(\
                          xmldocElem,['replicaList','replica','azimuthTime'])))
        ...
        GRD['pixel'], GRD['line'] = \
            np.meshgrid(range(numberOfSamples),range(numberOfLines))
        GRD['azimuthTime'] = \
            griddata(gridSampleCoord,geolocationGridPoint['azimuthTime'], \
                (GRD['line'], GRD['pixel']),method='linear')
        GRD['slantRangeTime'] = \
            griddata(gridSampleCoord,geolocationGridPoint['slantRangeTime'], \
                (GRD['line'],GRD['pixel']),method='linear')
        GRD['elevationAngle'] = \
            griddata(gridSampleCoord,geolocationGridPoint['elevationAngle'], \
                (GRD['line'],GRD['pixel']),method='linear')
        GRD['noiseLut'] = \
            np.ones((numberOfLines,numberOfSamples)) * np.nan
        GRD['descallopingGain'] = \
            np.ones((numberOfLines,numberOfSamples)) * np.nan

        swathMergeList = getElem(xmldocElem,['swathMergeList'])
        for iSwathMerge in swathMergeList.getElementsByTagName('swathMerge'):
            subswathID = getValue(getElem(iSwathMerge,['swath']))
            ...
            for iSwathBounds in swathBoundsList.getElementsByTagName('swathBounds'):
                firstAzimuthLine = int( \
                    getValue(getElem(iSwathBounds,['firstAzimuthLine'])))
                firstRangeSample = int( \
                    getValue(getElem(iSwathBounds,['firstRangeSample'])))
                lastAzimuthLine = int( \
                    getValue(getElem(iSwathBounds,['lastAzimuthLine'])))
                lastRangeSample = int( \
                    getValue(getElem(iSwathBounds,['lastRangeSample'])))
        ...
        sampleCoord = \
            np.array( ( np.dot( np.array(sigmaNought['line'])[np.newaxis].T,\
        ...
        GRD['sigmaNought'] = \
            griddata( sampleCoord, \
                     np.stack(sigmaNought['value']).flatten(), \
                     (GRD['line'], GRD['pixel']), method='linear' )

        DN = Nansat.__getitem__(self, 'DN_'+iPol)
        DN[DN==0] = np.nan
        if iPol=='HH':
            angularDependency = \
