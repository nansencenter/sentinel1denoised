...
# update npz files
update_npz_files = False

if update_npz_files:
    # trained coefficients from the noise scaling step
    #path_to_trained_npz = '/Home/denemc/s1denoise/sentinel1denoised/s1denoise/denoising_parameters_%s.npz' % platform
    path_to_trained_npz = '/Home/denemc/proc/noise_coeff_training/experimentalData/powerBalancing/pb_upd_denoising_parameters_%s.npz' % platform
    outfile_npz_file = 'es_upd_denoising_parameters_%s.npz' % platform

# import data
npzFilesAll = sorted(glob.glob('%s/%s_%s_GRDM_1SDH_*_extraScaling.npz' % (in_path, platform, mode)))
...
if update_npz_files:
    print('\ngoing to update coeffients for the extra balancing...')
    ...
    for i in range(1,num_ss[mode]+1):
        ss = '%s%d' % (mode, i)
        print(ss, d_s1['HV']['extraScalingParameters'][ss])
    print(d_s1['HV']['extraScalingParameters']['SNNR'])

    print('\nnew obtained coefficients')
    for i in range(1, num_ss[mode] + 1):
        ss = '%s%d' % (mode, i)
        print(ss, extraScalingParameters[ss])

        # replace values
        d_s1['HV']['extraScalingParameters'][ss] = extraScalingParameters[ss]
    print(extraScalingParameters['SNNR'])

    # SNNR
    d_s1['HV']['extraScalingParameters']['SNNR'] = \
        extraScalingParameters['SNNR']
